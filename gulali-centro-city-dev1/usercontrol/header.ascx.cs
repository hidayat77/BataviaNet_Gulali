        ...
        if (!IsPostBack)
        {
            ...
            if (notif.Rows.Count > 0)
            {
                for (int i = 0; i < notif.Rows.Count; i++)
                {
                    ...
                    {
                        DataTable LM = Db.Rs("select e.Employee_Full_Name, Leave_ID, Leave_StatusLeave from TBL_Employee e join TBL_Leave l on e.Employee_ID = l.Employee_ID where Leave_ID = '" + notif.Rows[i]["Leave_ID"].ToString() + "'");

                        DataTable LM_Lembur = Db.Rs("select e.Employee_Full_Name, l.Lembur_Status from TBL_Employee e join " + Param.Db2 + ".dbo.TBL_Lembur l on e.Employee_ID = l.Employee_ID where Lembur_ID = '" + notif.Rows[i]["Lembur_ID"].ToString() + "'");

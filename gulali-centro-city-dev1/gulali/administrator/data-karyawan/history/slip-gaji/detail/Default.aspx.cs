...
public partial class _admin_data_karyawan_history : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        link_href.Text = "<a href=\"" + Param.Path_Admin + "data-karyawan/history/slip-gaji/?id=" + id + "\" class=\"btn btn-custom dropdown-toggle waves-effect waves-light\">Kembali <span class=\"m-l-5\"><i class=\"fa fa-backward\"></i></span></a>";
        if (!IsPostBack)
        {
            //check int
            int number;
    ...
    protected void fill()
    {
        DataTable employee = Db.Rs("Select Employee_ID, Employee_Full_Name, Employee_Bank_AccNumber_Primary, Employee_JoinDate, Employee_DateOfBirth, b.Religion_Name_ID from TBL_Employee a left join TBL_Religion b on a.Religion_ID = b.Religion_ID where Employee_ID = '" + Cf.StrSql(id) + "'");

        DataTable employee_payroll = Db.Rs("Select Payroll_SalaryPPH, Payroll_SalaryBPJS, Payroll_NPWP_Number, b.PTKP_Status from TBL_Employee_Payroll a join " + Param.Db2 + ".dbo.TBL_PTKP b on a.PTKP_ID = b.PTKP_ID where Employee_ID = '" + Cf.StrSql(id) + "'");
        ...
        if (payroll.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < loop_takteratur.Rows.Count; i++)
            {
                ...
                y.Append("<tr>"
                            ...
                            + "<td>:</td>"
                            + "<td>" + Cf.money(sum_tunjangan_takteratur, true) + "</td>"
                          + "</tr>");
            }
        ...
        txt_thr.Text = Cf.money(thr_conv, true);
        txt_overtime.Text = Cf.money(overtime_conv, true);
        txt_homepay_gapok.Text = Cf.money(curr_salary, true);
        txt_bonus.Text = Cf.money(bonus_conv, true);
        txt_incentive.Text = Cf.money(incentive_conv, true);
        txt_unpaid_leave.Text = Cf.money(unpaidleave_conv, true);
        txt_gaji_dan_tunjangan.Text = Cf.money(gaji_dan_tunjangan, true);
        txt_bpjs_kesehatan_corp.Text = Cf.money(bpjs_kesehatan_corp, true);
        txt_bpjs_ketenagakerjaan_corp.Text = Cf.money(bpjs_ketenagakerjaan_corp, true);
        txt_allowance.Text = Cf.money(allowance_val, true);
        txt_kesehatan_empl.Text = Cf.money(bpjs_peg, true);
        txt_ketenagakerjaan_empl.Text = Cf.money(jht_peg, true);
        txt_biaya_jabatan.Text = Cf.money(biaya_jabatan, true);
        txt_deduction.Text = "<b>" + Cf.money(total_pengurang_penghasilan, true) + "</b>"; 
        total_teratur = gaji_dan_tunjangan + bpjs_kesehatan_corp + bpjs_ketenagakerjaan_corp;
        txt_penghasilan_teratur.Text = "<b>" + Cf.money(total_teratur, true) + "</b>";
        txt_bpjs_pribadi.Text = Cf.money(bpjs_pribadi_val, true);
        ...
        txt_total_tak_teratur.Text = Cf.money(total_tak_teratur, true) + "</b>";
        total_bruto = total_teratur + total_tak_teratur;
        total_netto = total_bruto - total_pengurang_penghasilan;
        txt_bruto.Text = "<b>" + Cf.money(total_bruto, true) + "</b>";
        txt_netto.Text = "<b>" + Cf.money(total_netto, true) + "</b>";
        txt_ptkp.Text = Cf.money(ptkp, true);
        txt_kena_pajak.Text = "<b>" + Cf.money(total_netto - ptkp, true) +  "</b>";
        txt_pph_periode_ini.Text = "<b>" + Cf.money(pph_periode_ini, true) + "</b>"; 
        txt_pph_sudah_dipotong.Text = //"<b>Rp " + String.Format("{0:#,0.00}", (pph_periode_ini * (Convert.ToInt64(month) - 1))) + "</b>"//
            "<b>" + Cf.money(pph_periode_ini * (Convert.ToInt64(month) - 1), true) + "</b>";
        txt_pph_pkp.Text = Cf.money(pph_periode_ini * 12, true);
        txt_pph_pkp1.Text = "<b>" + Cf.money(pph_periode_ini * 12, true) +  "</b>";
        ...
        if (tunjangan_.Rows.Count > 0)
        {
            for (int i = 0; i < tunjangan_.Rows.Count; i++)
            {
                ...
                x.Append("<tr>"
                            ...
                            + "<td>:</td>"
                            + "<td>" + Cf.money(tunjangan_val, true) + "</td>"
                          + "</tr>");
        ...
        txt_homepay_thr.Text = Cf.money(homepay_thr_conv, true);
        txt_homepay_overtime.Text = Cf.money(homepay_overtime_conv, true);
        txt_homepay_bonus.Text = Cf.money(homepay_bonus_conv, true);
        txt_homepay_incentive.Text = Cf.money(homepay_incentive_conv, true);
        txt_homepay_bpjs_total.Text = Cf.money(bpjs_peg + jht_peg, true);
        txt_homepay_unpaidleave.Text = Cf.money(unpaidleave_conv, true);
        txt_homepay_pph_periode_ini.Text = Cf.moneymin(pph_periode_ini, true) + "" ; 
        ...
        take_homepay.Text = Cf.money(homepay, true) ;
    }
            ...
            using (HtmlTextWriter hw = new HtmlTextWriter(sw))
            {
                ...
                using (MemoryStream memoryStream = new MemoryStream())
                {
                    ...
                    using (MemoryStream input = new MemoryStream(bytes))
                    {
                        ...
                        {
                            DataTable employee = Db.Rs("Select Employee_Full_Name, Employee_Bank_AccNumber_Primary, Employee_JoinDate, Employee_DateOfBirth from TBL_Employee where Employee_ID = '" + Cf.StrSql(id) + "' and Employee_ID != 1");

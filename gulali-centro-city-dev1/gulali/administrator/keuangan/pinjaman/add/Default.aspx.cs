...
public partial class _admin_keuangan_pinjaman : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        App.ProtectPageGulali(this, "Keuangan >> Pinjaman", "Add");
    ...
    {
        int nominal_pinjaman = Convert.ToInt32(txt_pinjaman.Text.Replace(".",""));
        int jumlah_bulan = Convert.ToInt32(ddl_cicilan_bulan.SelectedValue);

        double pembagi = 0;
        pembagi = Convert.ToDouble(nominal_pinjaman / jumlah_bulan);

        //txt_jumlah_perbulan.Text = pembagi.ToString();
        txt_jumlah_perbulan.Text = Cf.money(pembagi, true); 
    }
    ...
    protected void save_Click(object sender, EventArgs e)
    {
        ...
        string insert = "Insert into TBL_Pinjaman (Employee_ID, Pinjaman_Lama_Bulan, Pinjaman_DateFrom, Pinjaman_DateTo, Pinjaman_Pokok, Pinjaman_Pokok_Terbilang, Pinjaman_Angsuran_Perbulan, Pinjaman_Keperluan, Pinjaman_CreateById, Pinjaman_DateCreate, Pinjaman_DateUpdate ) values('" + Cf.StrSql(id_karyawan) + "','" + Cf.Int(cicilan_bulan) + "','" + Cf.StrSql(tgl_mulai) + "','" + Cf.StrSql(tgl_selesai) + "','" + Cf.StrSql(jumlah_pinjaman.Replace(".", "")) + "','" + Cf.StrSql(jumlah_terbilang) + "','" + Cf.StrSql(jumlah_cicilan) + "','" + Cf.StrSql(keperluan) + "','" + Cf.StrSql(App.Employee_ID) + "',getdate(),getdate())";

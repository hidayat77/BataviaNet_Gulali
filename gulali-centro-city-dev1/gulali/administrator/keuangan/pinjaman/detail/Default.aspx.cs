...
public partial class _admin_keuangan_pinjaman : System.Web.UI.Page
{
    ...
    protected void view (string id)
    {
        ...
        //Convert Tanggal Mulai & Tanggal Selesai
        string tgl_mulai = view_data.Rows[0]["Pinjaman_DateFrom"].ToString();
        string tgl_selesai = view_data.Rows[0]["Pinjaman_DateTo"].ToString();
        DateTime tgl_1 = Convert.ToDateTime(tgl_mulai);
        DateTime tgl_2 = Convert.ToDateTime(tgl_selesai);

        //Convert Pinjaman Pokok & Angsuran
        double pokok = 0, jml_angsuran = 0;
        pokok = Convert.ToDouble(view_data.Rows[0]["Pinjaman_Pokok"]);
        jml_angsuran = Convert.ToDouble(view_data.Rows[0]["Pinjaman_Angsuran_Perbulan"]);
        ...
        tanggal_mulai.Text = tgl_1.ToString("dd MMMM yyyy");
        tanggal_selesai.Text = tgl_2.ToString("dd MMMM yyyy");
        txt_pinjaman.Text = Cf.money(pokok, true);
        txt_terbilang.Text = view_data.Rows[0]["Pinjaman_Pokok_Terbilang"].ToString();
        txt_jumlah_perbulan.Text = Cf.money(jml_angsuran, true);

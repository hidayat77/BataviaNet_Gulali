...
public partial class _admin_setup_general : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        link_back.Text = "<a href=\"" + Param.Path_User + "department/kalender/kehadiran/\" class=\"btn btn-custom dropdown-toggle waves-effect waves-light\">Kembali <span class=\"m-l-5\"><i class=\"fa fa-backward\"></i></span></a>";
    ...
    {
        string id_employee = Cf.Lower(Cf.Str(rs.Rows[i][0]));
        string id_schedule = Cf.Str(rs.Rows[i][1]);

        string date_schedule = "";
        if (!string.IsNullOrEmpty(rs.Rows[i][2].ToString()))
        {
            DateTime date = Convert.ToDateTime(rs.Rows[i][2].ToString());
            date_schedule = date.ToString("yyyy-MM-dd").ToString();
        }

        bool x = false;
        int value = 0;
        DataTable check_temp = Db.Rs("select max(Schedule_Temp_ID) as maks from TBL_Schedule_Kalendar_Temp where Schedule_Temp_CreateBy ='" + Cf.StrSql(App.Employee_ID) + "'");
        ...
        try
        {
            ...
            string insert = "Insert into TBL_Schedule_Kalendar_Temp(Employee_ID, Schedule_Role_ID, Schedule_Temp_Date, Schedule_Temp_CreateDate, Schedule_Temp_CreateBy) values('" + Cf.Int(id_employee) + "','" + Cf.Int(id_schedule) + "','" + date_schedule + "',getdate(),'" + App.Employee_ID + "')";

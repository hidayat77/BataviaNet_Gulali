...
public partial class _user_kalender_department_cuti : System.Web.UI.Page
{
    ...
    public void preview(string num, string sum, string search)
    {
        ...
        DataTable rs = Db.Rs("select distinct a.Employee_ID, Employee_Full_Name from TBL_Employee a join " + Param.Db2 + ".dbo.TBL_Lembur b on a.Employee_ID = b.Employee_ID " + where + " order by Employee_Full_Name asc");
        if (rs.Rows.Count > 0)
        {
            double of = 0;
            ...
            if (dari > 0)
            {
                for (int i = dari - 1; i < sampai; i++)
                {
                    ...
                    DataTable count_req = Db.Rs2("Select Lembur_ID from TBL_Lembur where Lembur_Status != 0 and Lembur_Status != 1 and Lembur_Status != 3 and Lembur_Status != 6 and Employee_ID = " + id + "");
                    DataTable count_create = Db.Rs2("Select Lembur_ID from TBL_Lembur where Employee_ID= " + id + "");
                    DataTable count_approved = Db.Rs2("Select Lembur_ID from TBL_Lembur where Lembur_Status = 0 and Employee_ID = " + id + "");

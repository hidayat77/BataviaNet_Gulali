...
public partial class _user_kalender_department_cuti_detail : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            link_back.Text = "<a href=\"" + Param.Path_User + "department/kalender/cuti/\" class=\"btn btn-custom dropdown-toggle waves-effect waves-light\">Kembali <span class=\"m-l-5\"><i class=\"fa fa-backward\"></i></span></a>";
    ...
    {
        DataTable rs = Db.Rs("select e.Employee_Full_Name, convert(varchar,Leave_RequestDate_HalfDay, 107) as Leave_RequestDate_HalfDay, convert(varchar,Leave_RequestDateFrom, 107) as Leave_RequestDateFrom, convert(varchar, Leave_RequestDateTo, 107) as Leave_RequestDateTo, convert(varchar, Leave_RequestDate_HalfDay, 106) as Leave_RequestDate_HalfDay, Type_Desc, e.Employee_ID, l.Leave_ID, Leave_Half, Leave_StatusLeave from TBL_Leave l join TBL_Leave_Type b on l.Type_ID = b.Type_ID join TBL_Employee e on l.Employee_ID = e.Employee_ID where Employee_DirectSpv = " + Cf.StrSql(App.Employee_ID) + " and e.Employee_ID != 1 and l.Employee_ID = '" + id + "' order by Leave_ID desc");
        ...
        if (rs.Rows.Count > 0)
        {
            ...
            if (dari > 0)
            {
                for (int i = dari - 1; i < sampai; i++)
                {
                    ...
                    if (rs.Rows[i]["Leave_StatusLeave"].ToString() == "0") { style = "style=\"color:#FF0000;font-weight:bold;\""; }
                    ...
                    if (rs.Rows[i]["Leave_Half"].ToString() != "0")
                    {
                        ...
                        "<td style=\"width:35%;text-align:center;\"><a href=\"detail-leave?id=" + id + "&cn=" + leave_id + "\" >");
                    }
                    else
                    {
                        x.Append("<td style=\"width:20%\">" + rs.Rows[i]["Leave_RequestDateFrom"].ToString() + " -  " + rs.Rows[i]["Leave_RequestDateTo"].ToString() + "</td>"
                            + "<td style=\"width:35%;text-align:center;\"><a style=\"\" href=\"detail-leave?id=" + id + "&cn=" + leave_id + "\" >");
                    }

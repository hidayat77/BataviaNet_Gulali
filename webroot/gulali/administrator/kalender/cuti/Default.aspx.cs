...
public partial class _admin_kalender_cuti : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            App.ProtectPageGulali(this, "Kalender >> Cuti", "view");
        }
    }
    ...
    public void preview(string num, string sum, string search)
    {
        ...
        DataTable rs = Db.Rs("select distinct a.Employee_ID, Employee_Full_Name from TBL_Employee a join TBL_Leave b on a.Employee_ID = b.Employee_ID " + where + " and Leave_StatusLeave in ('2', '3', '4') order by Employee_Full_Name asc");
        if (rs.Rows.Count > 0)
        {
            double of = 0;
            ...
            if (dari > 0)
            {
                for (int i = dari - 1; i < sampai; i++)
                {
                    ...
                    DataTable count_req = Db.Rs("Select Leave_ID from TBL_Leave where Leave_StatusLeave = 4 and Employee_ID = " + id + "");
                    DataTable count_create = Db.Rs("Select Leave_ID from TBL_Leave where Leave_StatusLeave not in ('0', '5') and Employee_ID = " + id + "");

...
public partial class _admin_setup_posisi_edit : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack) 
        {
            ...
            if (result)
            {
                DataTable dba = Db.Rs("Select * from TBL_Position where Position_ID = " + Cf.StrSql(id_get) + "");
                if (dba.Rows.Count > 0) { position.Text = dba.Rows[0]["Position_Name"].ToString(); }
                else { Response.Redirect("/notFound/404.aspx"); }
            } else { Response.Redirect("/notFound/404.aspx"); }
        }
    }
    ...
    {
        string update = "Update TBL_Position set Position_Name='" + position.Text + "' where Position_ID = '" + Cf.StrSql(id_get) + "'";
        Db.Execute(update);
        Response.Redirect("../");
    }
}
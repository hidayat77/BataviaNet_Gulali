...
public partial class _admin_kalender_kehadiran : System.Web.UI.Page
{
    ...
    protected void Calendar_DayRender(object sender, DayRenderEventArgs e)
    {
        ...
        //Show Schedule
        DataTable Schedule = Db.Rs2("Select CONVERT(VARCHAR(10),Schedule_Date ,105) as Schedule_Date from TBL_Schedule_Kalendar");
        if (Schedule.Rows.Count > 0)
        {
            for (int i = 0; i < Schedule.Rows.Count; i++)
            {
                if (Schedule.Rows[i]["Schedule_Date"].ToString() == e.Day.Date.ToString("dd-MM-yyyy").ToString())
                {
                    ...
                    string outputs = "";
                    string tanggal = tanggal_schedule.ToString("ddMMyy");
                    if (tanggal_schedule < sekarang)
                    {
                        //abu2
                        outputs = "<a href=\"jadwal/?date="+tanggal+"\" style=\"background-color: #9eafbf !important; border: 1px solid #9eafbf !important; border-radius: 2em; padding: 6px 18px; color: white !important;\">Lihat Jadwal</a>";
                    }
                    else
                    {
                        //biru

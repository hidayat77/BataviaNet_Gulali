...
public partial class _data_karyawan_detail : System.Web.UI.Page
{
    ...
    protected void fill()
    {
        ...
        if (rs.Rows.Count > 0)
        {
            ...
            string tgl_lahir = rs.Rows[0]["Employee_DateOfBirth"].ToString();
            DateTime tgl_1 = Convert.ToDateTime(tgl_lahir);
            tanggal_lahir.Text = tgl_1.ToString("dd MMMM yyyy");
            ...
            //convert tanggal Mulai Kerja
            string tgl_mulai_kerja = rs.Rows[0]["Employee_StartofEmployment"].ToString();
            DateTime tgl_2 = Convert.ToDateTime(tgl_mulai_kerja);
            start_of_employment.Text = tgl_2.ToString("dd MMMM yyyy");

            //convert tanggal Mulai Kontrak
            string tgl_join = rs.Rows[0]["Employee_JoinDate"].ToString();
            DateTime tgl_3 = Convert.ToDateTime(tgl_join);
            contract_start.Text = tgl_3.ToString("dd MMMM yyyy");

            //convert tanggal Selesai Kontrak
            string tgl_end = rs.Rows[0]["Employee_EndDate"].ToString();
            DateTime tgl_4 = Convert.ToDateTime(tgl_end);
            contract_end.Text = tgl_4.ToString("dd MMMM yyyy");
            ...
            if (depend.Rows.Count > 0)
            {
                for (int a = 0; a < depend.Rows.Count; a++)
                {
                    ...
                    if (depend.Rows[a]["Dependent_Gender"].ToString() == "L") { dep_gender = "Laki-Laki"; } else if (depend.Rows[a]["Dependent_Gender"].ToString() == "P") { dep_gender = "Perempuan"; };
                    ...
                    //convert tanggal lahir
                    string tgl_lahir_dependent = depend.Rows[a]["Dependent_DateofBirth"].ToString();
                    DateTime tgl_dep = Convert.ToDateTime(tgl_lahir_dependent);
                    dep_dateofbirth = tgl_dep.ToString("dd MMMM yyyy");
            ...
            gajipph = Convert.ToDouble(rs.Rows[0]["Payroll_SalaryPPH"]);
            gajibpjs = Convert.ToDouble(rs.Rows[0]["Payroll_SalaryBPJS"]);
            salary_pph.Text = Cf.money(gajipph, true);
            salary_bpjs.Text = Cf.money(gajibpjs, true);
            ...
            //bool sim_A = Convert.ToBoolean(rs.Rows[0]["Employee_SIM_A"].ToString());
            //bool sim_C = Convert.ToBoolean(rs.Rows[0]["Employee_SIM_C"].ToString());

            //if (sim_A == true){ checkbox_sim_a.Checked = true; } else { checkbox_sim_a.Checked = false; }
            //if (sim_C == true){ checkbox_sim_c.Checked = true; } else { checkbox_sim_c.Checked = false; }
            ...
            {
                gaji = Convert.ToDouble(salary.Rows[0]["HistorySalary_value"]);
                //if (!string.IsNullOrEmpty(salary.Rows[0]["HistorySalary_value"].ToString()))
                //{ current_salary.Text = salary.Rows[0]["HistorySalary_value"].ToString();  
                //}
                current_salary.Text = Cf.money(gaji, true);
            }
